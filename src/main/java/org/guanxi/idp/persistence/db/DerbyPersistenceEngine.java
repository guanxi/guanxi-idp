...
public class DerbyPersistenceEngine extends JDBCPersistenceEngine {
   ...
   * (Java SE 6) or newer, JDBC drivers may be automatically loaded, making this
   * code optional.
   * 
   ...
   * environment, the Derby engine is being run by the network server framework.
   * 
   * In an embedded environment, any static Derby system properties must be set
   * before loading the driver to take effect.
   */
   ...
   * checkpoint when the JVM shuts down. This means that it will take longer to
   * boot (connect to) the database the next time, because Derby needs to
    ...
    if (framework.equals("embedded")) {
      try {
        ...
        if (((sqle.getErrorCode() == 50000) && ("XJ015".equals(sqle.getSQLState())))) {
          /*
           * We got the expected exception. Note that for single database
           * shutdown, the expected SQL state is "08006" and the error code is
           * 45000.
           */
          ...
          /*
           * if the error code or SQLState is different, we have an unexpected
           * exception (shutdown failed)
           */
          logger.error("Derby did not shut down normally", sqle);
        }
      }
    }
