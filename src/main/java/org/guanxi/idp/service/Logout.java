 ...
 * </p>
 * 
 * This filter provides an HTTP binding for logout functionality. It calls the
 * low level SSO API to do the actual logout. To use it, set the <url-pattern>
 * of the <filter-mapping> to be the logout URL of the application in which the
 * IdP is embedded. e.g. <url-pattern>/logout</url-pattern> The default value
 * will let users logout out of the standalone IdP.
 * 
...
public class Logout extends HandlerInterceptorAdapter implements ServletContextAware {
  ...
  /**
   * This handler traps logout requests to the IdP or the application in which
   * the IdP is embedded. If the IdP is standalone (passive = no), then the
   * filter will log the user out of the IdP and display the logout page as
   * specified in web.xml. If the IdP is embedded in an application (passive =
   * yes) then the filter will log the user out of the IdP and stand back and
   * allow the application to continue with it's logout functionality. The
   * filter will not display the logout page in this case.
   * 
   * @param request
   *          Standard HttpServletRequest
   * @param response
   *          Standard HttpServletResponse
   * @param object
   *          handler
   * @return true
   * @throws Exception
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object)
    ...
    /*
     * Continue with the request cycle. This depends on the passive setting in
     * web.xml. If we're not passive then we've already redirected to the logout
     * page and continuing with the filter chain will produce an
  ...
  public void processLogout(HttpServletRequest request, HttpServletResponse response)
      ...
      for (int c = 0; c < cookies.length; c++) {
        if (cookies[c].getName().equals(cookieName)) {
          ...
          GuanxiPrincipal principal =
                                      (GuanxiPrincipal) servletContext
                                                                      .getAttribute(cookies[c]
                                                                                              .getValue());
    ...
    if (!passive) {
      if (loggedOut)
        request.setAttribute("LOGOUT_MESSAGE", messageSource.getMessage("idp.logout.successful",
